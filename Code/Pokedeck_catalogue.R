...
packages <- c('easypackages','readxl', 'tidyr', 'dplyr', 'readr', 'jsonlite', 'httr', 'rvest', 'stringr', 'scales', 'ggplot2','sjmisc')
install.packages(setdiff(packages, rownames(installed.packages())))
easypackages::libraries(packages)

local_store <- '~/Repositories/Pokedeck/Data'

# source - https://www.denofgeek.com/games/pokemon-scarlet-violet-how-to-evolve-every-techniques-requirements-special-items/

# read_csv(paste0(local_store, '/pokemon_leveling_raw.csv')) %>% 
#   mutate(Evolves_from = substr(Data, 1, as.numeric(str_start(Data, '<')) - 2)) %>% 
#   mutate(Evolves_to = substr(Data, as.numeric(str_start(Data, '<')) + 2,as.numeric(str_start(Data, 'Lv')) - 2)) %>% 
#   mutate(Level = substr(Data, as.numeric(str_start(Data, 'Lv')) + 4, nchar(Data))) %>% 
#   select(!Data) %>% 
#   write.csv(., paste0(local_store, '/evolution_data_cleaned.csv'), row.names = FALSE)
         
# read_csv(paste0(local_store, '/pokemon_stones_raw.csv')) %>% 
#   mutate(Evolves_from = substr(Data, 1, as.numeric(str_start(Data, '(')) - 2)) %>% 
#   mutate(Evolves_to = substr(Data, as.numeric(str_start(Data, ')')) + 2, nchar(Data))) %>% 
#   mutate(Stone = substr(Data,  as.numeric(str_start(Data, '(')) +1,  as.numeric(str_start(Data, ')')) - 1)) %>% 
#   select(!Data) %>% 
#   write.csv(., paste0(local_store, '/evolution_stones_data_cleaned.csv'), row.names = FALSE)
